"""
Qwen2D VAE architecture — 2D version of the WanVAE for image generation.

This is a 2D VAE that uses the same 16-channel latent format as the WanVAE
but operates without the temporal dimension, making it faster and compatible
with models that use the WanVAE latent format (Krea2, Qwen, Anima) for
image generation.

Source: https://huggingface.co/Anzhc/Qwen2D-VAE
...
class Qwen2DVAE(nn.Module):
    """2D VAE compatible with WanVAE latent format (16 channels).

    Accepts 5D input [B, C, T, H, W] but flattens to 2D [B*T, C, H, W]
    internally, making it much faster than the 3D WanVAE for image generation.
    """
