...
_ENTRY = _ROOT / "__init__.py"
_NODE = _ROOT / "nodes" / "sega.py"


@pytest.mark.unit
class TestSegaNodeSchema:
    def _get_node_class(self):
        """Import the SEGA node class from the pack entry."""
        import importlib
    ...
    def test_node_class_exists(self):
        """SEGA node class should be defined in nodes/sega.py."""
    ...
    def test_imports_apply_sega(self):
        """nodes/sega.py should import apply_sega_to_model."""
        content = _NODE.read_text(encoding="utf-8")
    ...
    def test_node_category(self):
        """The SEGA node should be in the WMNodes/image category."""
        content = _NODE.read_text(encoding="utf-8")
